...
test('ToolCallingAgent.extractAction: returns null when no tool_calls', () => {
...
test('ToolCallingAgent.appendActionToHistory: records ONLY the executed tool_call (fix for parallel calls)', () => {
  const agent = makeAgent();
  const response = {
    content: null,
    tool_calls: [
  ...
  assert.equal(agent.history[0].tool_calls.length, 1, 'must store only the executed call, not all of them');
...
test('ToolCallingAgent.buildSystemPrompt: lists managed agents exactly once (no dup with tools list)', () => {
  const sub = makeAgent();
  const { ManagedAgent } = await import('../src/managed-agent.js');
...
test('ToolCallingAgent.getToolSchemas: includes managed agents alongside regular tools', async () => {
  const sub = makeAgent();
  const { ManagedAgent } = await import('../src/managed-agent.js');
  const ma = new ManagedAgent({ agent: sub, name: 'researcher', description: 'does research' });
  const t = tool({
